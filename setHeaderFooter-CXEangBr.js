...
function g(e){return e.find(a=>a.type==="Mailing");}
function h(e){return e.find(a=>a.type==="Voice").phoneNumber;}
function w(e){const t=g(e.addresses),a=h(e.contacts.phoneNumbers);return`<section class="contact">
    <h3>Contact Info</h3>
    ...
    `;}
function P(e){let t;switch(e.category){case"Park Closure":t="closure";break;default:t=e.category.toLowerCase();}return`<li class="alert">
  <svg class="icon" focusable="false" aria-hidden="true">
    <use xlink:href="${u}#alert-${t}"></use>
  </svg>
  <div>
    <h3 class="alert-${t}">${e.title}</h3>
    <p>${e.description}</p>
  </div></li>`;}
function k(e){return`<li class="visitor-center">
  <h4>${e.name}</h4>
  <p>${e.description}</p>
  <p>${e.directionsInfo}</p>
  </li>`;}
function C(e){return e.map(t=>`<li>${t.name}</li>`).join("");}
function f(e){const t=document.querySelector(".disclaimer > a");t.href=e.url,t.innerHTML=e.fullName,document.querySelector("head > title").textContent=e.fullName,document.querySelector(".hero-banner > img").src=e.images[0].url,document.querySelector(".hero-banner__content").innerHTML=m(e);}
function y(e){document.querySelector("#park-footer").insertAdjacentHTML("afterbegin",w(e));}
function T(e){f(e),y(e);}
export{B as a,v as b,b as c,P as d,C as e,D as g,A as m,T as s,k as v};
